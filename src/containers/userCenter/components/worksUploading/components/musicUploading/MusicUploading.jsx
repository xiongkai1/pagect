...
class MusicUploading extends React.Component {
    constructor(props) {
        super(props);   
    ...
    handleSubmit = (e) => {
        ...
        this.props.form.validateFields((err, values) => {
            if (!err) {
                ...
                const authorizationPrice = values.musicPriceInfoVoList.authorizationPrice;// 价格
                const authorizationPriceSell = values.musicPriceInfoVoList.authorizationPriceSell;// 价格
                const priceVoList =  values.musicPriceInfoVoList = [];
                ...
                mus.engineeringUrl = this.state.fileList[2];
                mus.fileSize = '';
                mus.uploadTime = '';
                values.musicWorkInformationVo = mus;
                // 添加商品
                shopMusicUpload(values).then(e => {
                    if (e.data.code === 200) {
    ...
    render() {
        ...
        const uploadButton = (
            <div>
                <Icon type="plus" />
                <div className="ant-upload-text">音乐文件(不要打包)</div>
                <div className="ant-upload-text">所上传文件不要有任何广告信息</div>
                <div className="ant-upload-text">不超过50MB</div>
            </div>
        );
        const uploadButton1 = (
            <div>
        ...
        const uploadButton3 = (
            <div>
                <Icon type="plus" />
                <div className="ant-upload-text">工程文件截图</div>
                <div className="ant-upload-text">不超过5MB,格式JPG</div>
            </div>
        );
        const props = {
        ...
        return (
            <div className={styles.musicUploading}>
                    ...
                    <Tabs>
                        <TabPane tab="作品设置" key="1">
                            <Form onSubmit={this.handleSubmit}>
                                ...
                                <Form.Item label="市场价">
                                    {getFieldDecorator('musicPriceInfoVoList.authorizationPrice', {
                                        rules: [{ required: true, message: '价格不能为空' }]
                                    })(<Input placeholder="按照授权，设置价格"/>)}
                                </Form.Item>
                                <Form.Item label="售价">
                                    {getFieldDecorator('musicPriceInfoVoList.authorizationPriceSell', {
                                        rules: [{ required: true, message: '价格不能为空' }]
                                    })(<Input placeholder="按照授权，设置价格"/>)}
                                </Form.Item>
                                <Form.Item label="分润">
                                ...
                                <Form.Item >
                                    <Button type="danger" htmlType="submit" disabled={this.state.percent === 0 && lout && fileList.length > 3  ? false : true}>上传</Button>
                                </Form.Item>
                            </Form>
                        </TabPane>
                    </Tabs>
...
const BaseInfo = Form.create()(MusicUploading);
